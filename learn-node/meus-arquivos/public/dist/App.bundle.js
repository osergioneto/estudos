...
function searchResultsHTML(stores) {
	return stores.map(function (store) {
		return '\n\t\t\t<a href="/store/' + store.slugs + '" class="search__result">\n\t\t\t\t<strong>' + store.name + '</strong>\n\t\t\t</a>\n\t\t';
	}).join('');
}
...
function typeAhead(search) {
	...
	searchInput.on('input', function () {
		...
		// Se não tiver nenhum valor, finaliza
		if (!this.value) {
		...
		_axios2.default.get('/api/v1/search?q=' + this.value).then(function (res) {
			...
			searchResults.innerHTML = _dompurify2.default.sanitize('<div class="search__result">Nenhum resultado para "' + _this.value + '" foi encontrado</div>');
		}).catch(function (err) {
			console.log(err);
		});
